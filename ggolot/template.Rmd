...
```{r}
ggplot(diamonds, aes(x = carat, y = price)) + 
  geom_point(Fill= "blue", color= "brown")
...
```{r}
obds_diamond <- ggplot(diamonds, aes(x = carat, y = price, color=cut)) + 
  geom_point() + geom_smooth?
...
ggplot(ChickWeight, aes(x=Time, y=weight, color=Diet)) +
  geom_point()+ geom_smooth()
ggplot(ChickWeight, aes(x=Time, y=weight, color=Diet)) +
  geom_point()+ geom_smooth(method = "m")
...
ggplot(ggplot2::msleep, aes(x=order)) + 
  geom_bar(aes(color=order))+
  theme(axis.text.x = element_text(angle = 90, hjust=1, vjust=0.5, size=rel(1.5))) + labs(title="msleep", x="order", y="observation")
...
```{r}
ggplot(ggplot2::msleep, aes(x=order)) + 
  geom_bar(aes(color=order, fill=genus), color="black")+
  theme(axis.text.x = element_text(angle = 90, hjust=1, vjust=0.5, size=rel(1.5))) + labs(title="msleep", x="order", y="observation") +
theme(legend.key.size = unit(2, "mm"))+
